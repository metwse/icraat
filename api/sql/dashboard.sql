...
RETURNS TABLE (exams integer[], publishers integer[], lessons integer[], "exams.categories" integer[], users integer[]) 
LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
	FOR __exam, __publisher, __category, __user IN SELECT exams.id, exams.publisher_id, exams.category_id, exams.user_id FROM exams GROUP BY exams.publisher_id, exams.category_id, exams.id ORDER BY exams.id DESC LIMIT 30 LOOP
		...
		IF NOT __user = ANY(__users) THEN __users := array_append(__users, __user); END IF; 
	END LOOP;
	...
	RETURN QUERY SELECT __exams, __publishers, __lessons, __exams_categories, __users;
END $$;
