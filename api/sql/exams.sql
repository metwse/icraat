...
RETURNS TABLE (id integer, publisher_id integer, category_id integer, name text, net numeric, nets numeric[], duration numeric, "timestamp" timestamp with time zone) 
LANGUAGE plpgsql AS $$
BEGIN
	RETURN QUERY SELECT exams.id, exams.publisher_id, exams.category_id, exams.name, exams.net, exams.nets, EXTRACT(epoch FROM exams.duration) / 60, exams.timestamp FROM exams WHERE exams.id = _id;
END $$;
...
CREATE OR REPLACE FUNCTION exams.new(_details text, _nets integer[], _duration interval, _timestamp timestamp with time zone DEFAULT NOW())
RETURNS integer 
LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
	...
	IF __category IS null OR NOT EXISTS (SELECT 1 FROM publishers WHERE publishers.id = __publisher_id) OR _duration > '1 day'::interval  THEN RETURN 0;
	ELSE
		...
		ELSE
			...
			INSERT INTO exams (publisher_id, category_id, net, duration, timestamp, nets, name, full_name) VALUES (__publisher_id, __category_id, __net, _duration, _timestamp, __nets, __name, __full_name) RETURNING id INTO __a;
			RETURN __a;
		END IF;
	END IF;
END $$;
